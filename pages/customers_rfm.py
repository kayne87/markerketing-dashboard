...
def get_rfm_stats():
    ...
    #for each customers we want to know how many products they have in their orders (avg estimation)
    df_order_j_avgproducts = df_orders_j.groupby(by=["order_id", "customer_id"])[["product_id"]].count().reset_index()[["customer_id", "product_id"]].groupby(by=["customer_id"]).mean().reset_index().rename(columns={"product_id": "Products in ord."})
    df_order_j_avgorders= df_orders_j[df_orders_j["gross_price"] > 0].groupby(by=["order_id", "customer_id"])[["gross_price"]].sum().reset_index().groupby(by="customer_id")[["gross_price"]].mean().reset_index().rename(columns={"gross_price": "Average bill"})

    df_rfm_stats = pd.concat([
        df_rfm[["CLASS", "Recency", "Monetary"]].groupby(by=["CLASS"]).median().rename(columns={
            "Recency": "Recency (med.)",
            "Monetary": "Monetary (med.)"
        }).transpose(),
        df_rfm[["CLASS", "Frequency"]].groupby(by=["CLASS"]).mean().rename(columns={
            "Frequency": "Frequency (med.)"
        }).transpose()
        , pd.merge(
            df_order_j_avgproducts,
        ...
        , pd.merge(
            ...
            on="customer_id",
            how='left'
        )[["Average bill", "CLASS"]].groupby(by=["CLASS"]).mean().transpose()
    ]).round(2)
    return df_rfm_stats

def get_rfm_monetary():
    global df_rfm
    df_grouped = df_rfm[["CLASS", "Recency", "Frequency", "Monetary"]].groupby(by=["CLASS"]).median().sort_values(["Monetary"])
